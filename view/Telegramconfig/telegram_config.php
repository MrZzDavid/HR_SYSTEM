    ...
    <tbody id="data">
        ...
        if ($result && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $chat_name   = isset($row['chat_name']) ? $row['chat_name'] : '';
                $chat_id     = isset($row['chat_id']) ? $row['chat_id'] : '';
                $bot_token   = isset($row['bot_token']) ? $row['bot_token'] : '';
                $description = isset($row['description']) ? $row['description'] : '';
                $status      = isset($row['status']) ? (int)$row['status'] : 0;

                // Mask token
                $masked_token = $bot_token ? substr($bot_token, 0, 10) . '...' : '';
        ?>
                <tr data-id="<?php echo htmlspecialchars($id); ?>">
                    <td>
                        <button
                            class="btn btn-primary btn-sm edit-telegram-btn"
                            data-id="<?php echo htmlspecialchars($id); ?>"
                            data-chat-name="<?php echo htmlspecialchars($chat_name); ?>"
                            data-chat-id="<?php echo htmlspecialchars($chat_id); ?>"
                            data-bot-token="<?php echo htmlspecialchars($bot_token); ?>"
                            data-description="<?php echo htmlspecialchars($description); ?>"
                            data-status="<?php echo $status; ?>">
                            <i class="fas fa-edit"></i> Edit
                        </button>
                        <button
                            class="btn btn-danger btn-sm delete-telegram-btn"
                            data-id="<?php echo htmlspecialchars($id); ?>">
                            <i class="fas fa-trash"></i> Delete
                        </button>
                    </td>
                    <td><?php echo htmlspecialchars($chat_name); ?></td>
                    <td><?php echo htmlspecialchars($chat_id); ?></td>
                    <td><?php echo htmlspecialchars($masked_token); ?></td>
                    <td><?php echo htmlspecialchars($description); ?></td>
                    <td>
                        <center>
                            <div class="form-check form-switch">
                                <input type="checkbox" class="form-check-input" <?php echo $status === 1 ? 'checked' : ''; ?> disabled>
                            </div>
                        </center>
                    </td>
                </tr>
